...
**Private chat over Tor — terminal and Android.** Talk one to one with nobody
in the middle, or run your own server with Discord-style channels on a machine
...
<p align="center">
  ...
  <a href="https://github.com/KairoWolf/tor2/actions/workflows/android.yml">
    <img alt="android" src="https://github.com/KairoWolf/tor2/actions/workflows/android.yml/badge.svg">
  </a>
</p>
...
6. [Privacy: what is and isn't protected](#6-privacy-what-is-and-isnt-protected)
...
A native app that speaks the same protocol, so it joins the same servers with
no bridge in between.

```sh
adb install -r dist/tor2-0.7.1.apk
```

Or copy the APK to the phone and tap it. Tor runs on the phone; the first
start takes 30–60 seconds.

- **Join** with an 8-digit code, or an address and invite.
- **Direct chats** work both ways — the phone publishes its own address, and
  pairs with 5-digit codes.
- **Send** pictures, video and audio with **+**. Oversized pictures are shrunk;
  videos carry a still so people see what something is first.
- **Watch and listen inside the app.** Tapping a video plays it in tor2.
  Media stays in the app's private storage and is never handed to another app —
  the app has no file provider at all, so nothing you receive can reach your
  gallery or any other app on the phone.
- **Admins** get a Manage server sheet: mint codes, add or delete channels,
  kick, ban, promote.
- Hold a server for its address, copy and share buttons, and **Leave**.

The app's cryptography is not a reimplementation: it uses libsodium, and its
tests check key derivation, the frame layout and code-to-address derivation
against values generated by the Python client, so the two cannot silently
diverge. See [android/README.md](android/README.md).
...
## 6. Privacy: what is and isn't protected
...
cd android && ./gradlew test       # phone crypto against desktop vectors
```

Both suites run in CI on every push and never touch the network or your real
