...
# ---------- a `tor2-server` on PATH ----------
#
# The real entry point lives inside the venv, which is not on anyone's PATH,
# so admin commands from the README used to fail with "command not found".
# This shim fixes that and defaults the data directory, so minting a code is
# `tor2-server --joincode` from any directory rather than two long paths.
...
SHIM="$SHIM_DIR/tor2-server"

if mkdir -p "$SHIM_DIR" 2>/dev/null; then
  cat > "$SHIM" <<SHIMEOF
#!/usr/bin/env bash
# Generated by install-server.sh — runs the tor2 server from its virtualenv.
# With no data directory given, the one this server was installed with is used.
exec $SERVER_BIN "\${TOR2_DATA_DIR:-$DATA_DIR}" "\$@"
SHIMEOF
  chmod 755 "$SHIM"
  say "installed 'tor2-server' command at $SHIM"
  case ":$PATH:" in
    *":$SHIM_DIR:"*) ;;
    *) warn "$SHIM_DIR is not on your PATH — call it as $SHIM" ;;
  esac
else
  warn "could not write $SHIM_DIR; call the server as: $SERVER_BIN $DATA_DIR"
  SHIM="$SERVER_BIN $DATA_DIR"
fi
